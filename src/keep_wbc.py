...
def filter_tiles(model: SimpleCNN, tfms: T.Compose, threshold: float, dry_run: bool):
    total = kept = preview_count = 0
    for slide_dir in tqdm(list_slide_dirs(), desc="Filtering tiles"):
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Only preview; do not delete tiles.")
    args = parser.parse_args()

    train_loader, _ = load_debug_datasets(train=True)
    val_loader, val_tfms = load_debug_datasets(train=False)
    model = train_model(train_loader, val_loader)
    filter_tfms = val_tfms
    filter_tiles(model, filter_tfms, args.threshold, dry_run=args.dry_run)
